...
lp$year2 <- lp$year-(min(lp$year)-1) # add minus one here if reverting dd model
# add sites
...
yrind.nest <- array(NA, dim = c(max(nest.end), (nyr-1), nsite-1) )
for (t in 1:(nyr-1)){
  for (s in 1:(nsite-1)){
...
yrind.brood <- array(NA, dim = c(max(brood.end), (nyr-1), nsite-1) )
for (t in 1:(nyr-1)){
  for (s in 1:(nsite-1)){
...
num.treated <- cbind(colSums(treatLH, na.rm=T), colSums(treatPC, na.rm=T) )
rownames(num.treated) <- as.numeric(rownames(num.treated))-1

# ALTERNATIVE WAYS TO CALCULATE NUM TREATED
nt2 <- tapply(lp$treat, list(lp$ter, lp$year, lp$site), max, na.rm=T) |> 
  apply(c(2, 3), sum, na.rm=T)
#cbind(num.treated, nt2)
# lt <- melt(treat)
# df.site <- df.all[!duplicated(df.all$territory.name),]
# lt<- merge(lt, df.site[, c(2,6)], 
#            by.x="Var1", by.y="territory.name", all.x=T)
# lt <- lt[lt$value==1 & !is.na(lt$value), ]
# num.treated <- table(lt$Var2-1, lt$current.population)
...
# into 1 array
pPrior <- array(NA, dim=c(10,nrow(pPriorAD),2))
# N[1,2,5,] are capped by NFY, so it can't exceed
pPrior[1,1:s.endFY[1],1] <- pPrior[2,1:s.endFY[1],1] <- 
  pPrior[3,1:s.endFY[1],1] <-
  pPrior[7,1:s.endFY[1],1] <- pp1FY
pPrior[4,1:s.endAD[1],1] <- pPrior[5,1:s.endAD[1],1] <-  
  pPrior[8,1:s.endAD[1],1] <- pPrior[9,1:s.endAD[1],1] <- 
  pp1AD

pPrior[1,1:s.endFY[2],2] <- pPrior[2,1:s.endFY[2],2] <- 
  pPrior[3,1:s.endFY[2],2] <- 
  pPrior[7,1:s.endFY[2],2] <- pp2FY
pPrior[4,1:s.endAD[2],2] <- pPrior[5,1:s.endAD[2],2] <-
  pPrior[8,1:s.endAD[2],2] <- pPrior[9,1:s.endAD[2],2] <- 
  pp2AD

s.end <- array(NA, dim=c(10,2))
# N[1,2,5,] are capped by NFY
s.end[1,1] <- s.end[2,1] <- 
  s.end[3,1] <- s.end[7,1] <- s.endFY[1]
# N[3,4,6,7,] are capped by adults
s.end[4,1] <- s.end[5,1] <-
   s.end[8,1] <-s.end[9,1] <- s.endAD[1]

s.end[1,2] <- s.end[2,2] <- 
  s.end[3,2] <-  s.end[7,2] <- s.endFY[2]
s.end[4,2] <- s.end[5,2] <-
  s.end[8,2] <-s.end[9,2] <- s.endAD[2]

# deal with immigrants
# set to zero here
s.end[c(6,10), 1:2] <- 1 # length of one
pPrior[c(6,10), 1, 1:2] <- 1 # prob of 1

# get the first y values
y.first <- c()
for (i in 1:nind){
  y.first[i] <- y[i,frst[i]]  
}

# Setup hacked data
# PROBLEM BECAUSE LH OVERESTIMATES HACKED IN EARLY YEARS
# BC NB IS NOT KNOWN AND ONLY SOME ARE DETECTED
lh.df <- df[df$origin=="LHNP" & df$Stage==1, ] 
hacked.lh <- ifelse(lh.df$history=="Hack", 1, 0)
pc.df <- df[df$current.population=="PC" & df$Stage==1 & !is.na(df$current.population), ]
hacked.pc <- ifelse(lh.df$history=="Hack" & lh.df$current.population=="PC", 1, 0)
...
constl <- list( # survival 
                ...
                lh.year = lh.df$year.resighted-2010,
                pc.year = pc.df$year.resighted-2010,
                nhacked.lh = length(hacked.lh),
                nhacked.pc= length(hacked.pc)
)
...
yrind.pair <- array(NA, dim = c(max(constl$pair.end), (constl$nyr-1), constl$nsite) )
...
save(datl, constl,
     ...
     inits.func1, inits.func.pva, 
     z, seeds, 
     file="data\\data-pb.rdata")
