...
rm(list=c("post", "mycode"))
load("/bsuscratch/brianrolek/riha_ipm/data-dd.rdata")
cpus <- 20
for (jj in 1:length(par_info)){
par_info[[jj]]$cpu.num <- jj
}
...
run_ipm <- function(info, datl, constl, code, outp){
  ...
  n.chains=1; n.thin=1; n.iter=3; n.burnin=1
  ...
  inits.func <- function (){
    list(  
      ...
      N = outp$N[1:7,1:13,1:2,
                 sample(seq(1, 2500, by=1), 1, replace = F)], # sample from inits of chains that worked
...
while(failed | iter<=1000){
    ...
    for (h in 1:nrow(wfailed)){
      inits$N[wfailed$dim1[h], wfailed$dim2[h], wfailed$dim3[h]] <- outp$N[ wfailed$dim1[h], wfailed$dim2[h], wfailed$dim3[h], new.ind]
    } # h
    cmod$setInits( inits )
...
print("Found initials values. Beginning model run.")

if(iter<1000){
  flnm <- paste0("/bsuscratch/brianrolek/riha_ipm/outputs/ipm_findinits_cpu", info$cpu.num, ".rdata")
...
#       mus = outp2$mus[,,1], 
#       betas = outp2$betas[,1],
#       deltas = outp2$deltas[,1],
#       alphas = outp2$alphas[,1],
#       sds = outp2$sds[,1],
#       Ustar = outp2$Ustar[,,1],
#       # counts
#       r = outp2$r[,1],
#       N = outp2$N[1:7, 1:13, 1:2, 1], # sample from inits of chains that worked
#       z.score = outp2$z.score[,,,1],
#       prod = c(rep(NA, constl$npairsobs), 
#                rpois(n=constl$npairstot-constl$npairsobs, 
#                      lambda=ifelse(constl$site.pair[(constl$npairsobs+1):constl$npairstot]==1,
#                                    exp(-0.98), exp(-6.68))))
#     )}
#  
#   #n.chains2=1; n.thin2=100; n.iter2=100000; n.burnin2=50000
#   #n.chains2=1; n.thin2=100; n.iter2=250000; n.burnin2=150000
#   n.chains2=1; n.thin2=1; n.iter2=11; n.burnin2=1
#   
#   post3 <- runMCMC(chmc,
#                   niter = n.iter2, 
#                   nburnin = n.burnin2,
...
flnm2 <- "/bsuscratch/brianrolek/riha_ipm/outputs/ipm_findinits_all-cpus.rdata"
save(post3, file = flnm2)
# 
# save(post, mycode,
