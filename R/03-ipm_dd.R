...
mycode <- nimbleCode(
  {
    ...
    for (q in 1:10){
      deltas[q] ~ dunif(-20, 20) # prior for survey effort coefficients
    } # q
    for (qq in 1:6){
      for (s in 1:nsite){
      alphas[qq,s] ~ dunif(-20, 20)
    }} # qq s
    ...
    for (t in 1:nyr){
      for (s in 1:nsite){
        ...
        log(lamAD[t, s]) <- log(NAD[t, s]) + 
                            deltas[1]*effort2[t, s] + deltas[2]*effort2[t, s]^2 +
                            deltas[9]*effort.shift[t,s]
        log(lamFY[t, s]) <- log(N[1, t, s]) + 
                            deltas[3]*effort2[t, s] + deltas[4]*effort2[t, s]^2+
                            deltas[10]*effort.shift[t,s]
        }# s
    ...
    for (i in 1:nind){
      for (t in 1:nyr){
        ...
        logit(phiFY[i,t]) <- eta[1, site[i,t],t] + # eps[1,t] + 
          ...
          alphas[1, site[i,t]]*(Ntot[t, site[i,t]]-mnC[site[i,t]])  # first year
        logit(phiA[i,t]) <- eta[2, site[i,t],t] + #eps[2,t] + 
          lmus[2, site[i,t]] +  betas[2]*hacked[i]+
          alphas[2, site[i,t]]*(Ntot[t, site[i,t]]-mnC[site[i,t]]) # nonbreeder
        logit(phiB[i,t]) <- eta[3, site[i,t],t] + #eps[3,t] + 
          lmus[3, site[i,t]] + betas[3]*hacked[i]+
          alphas[3, site[i,t]]*(Ntot[t, site[i,t]]-mnC[site[i,t]]) # breeder
        #Recruitment
        logit(psiFYB[i,t]) <- eta[4, site[i,t],t] + #eps[4,t] + 
          lmus[4, site[i,t]] + betas[4]*hacked[i]+
          alphas[4, site[i,t]]*(Ntot[t, site[i,t]]-mnC[site[i,t]]) # first year to breeder
        logit(psiAB[i,t]) <- eta[5, site[i,t],t] + #eps[5,t] + 
          lmus[5, site[i,t]] + betas[5]*hacked[i]+
          alphas[5, site[i,t]]*(Ntot[t, site[i,t]]-mnC[site[i,t]]) # nonbreeder to breeder
...
save(post, mycode,
     file="/bsuscratch/brianrolek/riha_ipm/outputs/ipm_shortrun_dd.rdata")