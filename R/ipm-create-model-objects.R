...
# outp <- screen(post.inits)
# rm(list=c("mycode", "post.inits"))
...
mycode <- nimbleCode(
  {
    ...
    for (t in 1:(nyr-1)){ # this index for dens dep ipm bc mn.prod[t-1] would cause a zero index
      for (s in 1:nsite){
        ...
        #num.prod[t, s] ~ dpois( mean.dyn( prodmat[1:358, t, s], NB[t,s] ) * NB[t,s])
      }} # s t
  ...
  #flnm <- "/bsuscratch/brianrolek/riha_ipm/outputs/ipm-setup.rdata"
  flnm <- "C:\\Users\\rolek.brian\\OneDrive - The Peregrine Fund\\Documents\\Projects\\Ridgways IPM\\outputs\\ipm-setup.rdata"
  save(list= c("screen", "params", "mean.dyn", "outp", "inits.func", 
       "mycode", "mod", "cmod", "confhmc", "hmc", "chmc"), file = flnm)