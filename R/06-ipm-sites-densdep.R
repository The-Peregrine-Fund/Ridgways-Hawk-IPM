...
mycode <- nimbleCode(
  {
    ...
    ###################################################
    # Prior for density dependence of demographic rates
    for (j in 1:p){ 
      for (s in 1:nsite){
        deltas[p,s] ~ dunif(-10, 10)
      }} # s j
    # Prior for effects from translocation/hacking
    for (k in 1:8){ 
    ...
    for (k in 1:nnest){
      ...
      log(mu.f[k]) <- lmu.f[site.nest[k]] +  
        ...
        (Ntot[year.nest[k], site.nest[k]]-mean.ntot[site.nest[k]])*
        deltas[9, site.nest[k]]
    } # n 
    # derive yearly brood size for population model
    for (t in 1:nyr){
      for (s in 1:nsite){
        for (xxx in 1:nest.end[t,s]){
          fecmat[t,s,xxx] <- nu[yrind.nest[xxx,t,s]]
        } # xxx
      ...
      for (s in 1:nsite){
        ...
        N[1, t+1, s] ~ dpois( (NFY[t, s]*mn.phiFY[t, s]*mn.psiFYB[t, s] + # first year breeders
                              ...
                              *mn.f[t+1, s] ) # end Poisson
                              # productivity counts here only include females hence no mn.f/2
    ...
    for (i in 1:nind){
      for (t in 1:nyr){
        ...
        logit(phiFY[i,t]) <- eta[1, site[i,t],t] + eps[1,t] + 
                              lmus[1, site[i,t]] + betas[1]*hacked[i]  +
                              (Ntot[t,site[i,t]]-mean.ntot[site[i,t]])*deltas[1, site[i,t]]# first year
        logit(phiA[i,t]) <- eta[2, site[i,t],t] + eps[2,t] + 
                              lmus[2, site[i,t]] +  betas[2]*hacked[i] +
                              (Ntot[t,site[i,t]]-mean.ntot[site[i,t]])*deltas[2, site[i,t]]# nonbreeder
        logit(phiB[i,t]) <- eta[3, site[i,t],t] + eps[3,t] + 
                              lmus[3, site[i,t]] + betas[3]*hacked[i] +
                              (Ntot[t,site[i,t]]-mean.ntot[site[i,t]])*deltas[3, site[i,t]]# breeder
        #Recruitment
        logit(psiFYB[i,t]) <- eta[4, site[i,t],t] + eps[4,t] + 
                                lmus[4, site[i,t]] + betas[4]*hacked[i] +
                                (Ntot[t,site[i,t]]-mean.ntot[site[i,t]])*deltas[4, site[i,t]]# first year to breeder
        logit(psiAB[i,t]) <- eta[5, site[i,t],t] + eps[5,t] + 
                              lmus[5, site[i,t]] + betas[5]*hacked[i] +
                              (Ntot[t,site[i,t]]-mean.ntot[site[i,t]])*deltas[5, site[i,t]]# nonbreeder to breeder
        logit(psiBA[i,t]) <- lmus[6, site[i,t]] # breeder to nonbreeder
        #Re-encounter
...
save(post, mycode,
     file="/bsuscratch/brianrolek/riha_ipm/outputs/ipm_sites_densdep.rdata")
