...
#load("/bsuscratch/brianrolek/riha_ipm/outputs/ipm_findinits_all-cpus.rdata")
...
# #load("C:\\Users\\rolek.brian\\OneDrive - The Peregrine Fund\\Documents\\Projects\\Ridgways IPM\\outputs\\ipm_findinits_all-cpus.rdata")
...
mycode2 <- nimbleCode(
  {
    ...
    for (k in 1:npairstot){
      ...
      log(mu.prod[k]) <- lmu.prod[site.pair[k]] +  
        gamma*treat.pair[k] +
        alphas[6, site.pair[k]]*(Ntot[year.pair[k], site.pair[k]]-mnC[site.pair[k]]) +
...
run_ipm <- function(info, datl, constl, code, outp){
  ...
  #nc=1; nt=100; ni=250000; nb=150000
  # create a nimble function to handle dynamic indices
  mean.dyn <- nimbleFunction(
    run = function(mat = double(1), end = integer()) {
      return( mean(mat[1:end]) )
      returnType(double())
    })
  assign('mean.dyn', mean.dyn, envir = .GlobalEnv)

# Finding proper initial values are difficult for this model
# So here we use estimates from previous runs that worked
# along with some randomization and brute force
# Pseudocode
  # 1. Use iterations from previous working run and extract for some inits
  # 2. Randomly guess inits
  # 3. Run model for a very short number of iterations
  # 4. Test if the "N"s are reasonable values, not NAs or negative values
  # 5. If the "N"s broke, reset initial values and rerun.
  # 6. If the "N"s work, extract model values for a long model run
# initial values  
  inits.func <- function (){
    ...
    list(
      ...
      deltas = outp$deltas[,ind],
      alphas = array(runif(6*2, -0.1, 0.1), dim=c(6, 2) ),
      sds = outp$sds[,ind],
      Ustar = outp$Ustar[,,ind],
      # counts
      r = outp$r[,ind],
      N = outp$N[1:7, 1:13, 1:2, ind], # sample from inits of chains that worked
      z.score = outp$z.score[,,,ind],
      prod = c(rep(NA, constl$npairsobs),
               rpois(n=constl$npairstot-constl$npairsobs,
                     lambda=ifelse(constl$site.pair[(constl$npairsobs+1):constl$npairstot]==1,
                                   exp(-0.98), exp(-6.68))))
    )}
  inits <- inits.func()  
  ...
  post2 <- runMCMC(chmc,
                   ...
                   setSeed = info$seed,
                   inits= inits)
# Search for working inits
  iter <- 1
  failed <- TRUE
  test.p <- outp
while(failed | iter<=1000){
    # wfailed <- data.frame(which(is.na(test.p$N[,,,5]) | test.p$N[,,,5]<0, arr.ind=TRUE))
    # wfailed$nms <- paste0("N[", wfailed$dim1,", ", wfailed$dim2,", ", wfailed$dim3, "]")
    # new.ind <- sample(seq(1, 2500, by=1), 1, replace = F)
    # new.vals <- c(rep(NA, length(wfailed$nms)) )
    # names(new.vals) <- wfailed$nms
    # if (nrow(wfailed)==0){ next}
      
    # for (h in 1:nrow(wfailed)){
    #   inits$N[wfailed$dim1[h], wfailed$dim2[h], wfailed$dim3[h]] <- 
    #     outp$N[ wfailed$dim1[h], wfailed$dim2[h], wfailed$dim3[h], new.ind]
    # } # h
    # cmod$setInits( inits )
    ...
    test.p <- MCMCpstr(post2, type="chains")
    failed <- any (is.na(test.p$N[,,,5]) | test.p$N[,,,5]<0)
    iter <- iter+1
  } # while
...
#} # function end
 # if(iter<1000){
#   flnm <- paste0("/bsuscratch/brianrolek/riha_ipm/outputs/ipm_findinits_cpu-noN", info$cpu.num, ".rdata")
#   save(post2, file = flnm)
# }
# 
#   out2 <- lapply(post2, as.mcmc)
#   outp2 <- MCMCpstr(out2, type="chains")
# # initial values  
# inits.func2 <- function (){
#   # sample from working inits
#   ind <- sample(1:ncol(outp2$gamma), size=1, replace=TRUE)
#   list(
#     # fecundity
#     lmu.prod = outp2$lmu.prod[,ind],
#     gamma = outp2$gamma[,ind],
#     rr = outp2$rr[,ind] ,
#     # survival
#     z = info$inits$z,
#     mus = outp2$mus[,,ind],
#     betas = outp2$betas[,ind],
#     deltas = outp2$deltas[,ind],
#     alphas = outp2$alphas[,,ind],
#     sds = outp2$sds[,ind],
#     Ustar = outp2$Ustar[,,ind],
#     # counts
#     r = outp2$r[,ind],
#     N = outp2$N[1:7, 1:13, 1:2, ind], # sample from inits of chains that worked
#     z.score = outp2$z.score[,,,ind],
#     prod = outp2$prod[,ind]
#     )}
# #  
# nc=1; nt=100; ni=100000; nb=50000
#nc=1; nt=100; ni=250000; nb=150000
nc=1; nt=1; ni=1000; nb=500
# 
  post3 <- runMCMC(chmc,
                  niter = ni,
                  nburnin = nb,
...
flnm2 <- "/bsuscratch/brianrolek/riha_ipm/outputs/ipm-1000i.rdata"
